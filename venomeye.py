...
def _check_cors(live_file: str, out_dir: str):
    ...
    Two important fixes vs. the prior version:
      1. Use a real GET (`-X GET -D - -o /dev/null`) instead of HEAD (`-I`).
         Many servers omit CORS headers on HEAD or expose different ones,
         which caused both false negatives and noisy output.
      2. Drop the `*` + `Allow-Credentials: true` finding — every modern
         browser rejects that combination, so it is not exploitable.

    Adds three real-world bypass probes — arbitrary origin reflection,
    `null` origin, and the `target.evil.com` suffix bypass.
    """
    ...
    try:
        ...
        warn("CORS check: live hosts file missing")
        ok("CORS misconfigurations: 0")
        return

    def _fetch_headers(url: str, origin: str) -> str:
        try:
            r = subprocess.run(
                ["curl", "-sk", "-X", "GET", "-D", "-", "-o", "/dev/null",
                 "--max-time", "10", "-A", "Mozilla/5.0",
                 "-H", f"Origin: {origin}", url],
    ...
    for url in hosts:
        ...
        probes = [
            ("reflect",        "https://evil.com"),
            ("null",           "null"),
            ("suffix_bypass",  f"https://{host}.evil.com" if host else ""),
        ]

        for label, origin in probes:
            if not origin:
                continue
            headers = _fetch_headers(url, origin)
            if not headers:
                continue
            acao = _hdr(headers, "Access-Control-Allow-Origin")
            acac = _hdr(headers, "Access-Control-Allow-Credentials").lower() == "true"
            if not acao:
                continue

            if label == "reflect" and "evil.com" in acao.lower():
                tag = "CORS:REFLECT_CREDS" if acac else "CORS:REFLECT"
                findings.append(f"[{tag}] {url} | ACAO={acao} | ACAC={acac}")
                break
            if label == "null" and acao.lower() == "null":
                tag = "CORS:NULL_CREDS" if acac else "CORS:NULL"
                findings.append(f"[{tag}] {url} | ACAO={acao} | ACAC={acac}")
                break
            if label == "suffix_bypass" and host and host in acao.lower() and "evil.com" in acao.lower():
                findings.append(f"[CORS:SUFFIX_BYPASS] {url} | ACAO={acao} | ACAC={acac}")
                break
...
# ══════════════════════════════════════════════════════════════
# SSTI payloads use 8-digit results (7777*7777=60481729) so we don't false-fire
# on the digits "49" that turn up everywhere in HTML/CSS/JS. The detector also
# requires that the payload itself not be reflected verbatim — a reflection
# means the engine never evaluated it.
SSTI_PAYLOADS = [
    ("{{7777*7777}}",     "60481729", "Jinja2/Twig"),
    ("${7777*7777}",      "60481729", "FreeMarker"),
    ("#{7777*7777}",      "60481729", "Thymeleaf"),
    ("<%= 7777*7777 %>",  "60481729", "ERB"),
    ("${{7777*7777}}",    "60481729", "Spring SPEL"),
]
# Leak-style probes need anchors that are not common English words.
SSTI_LEAK_PAYLOADS = [
    ("{{config.items()}}",                    r"SECRET_KEY['\"]?\s*[:,]",     "Jinja2 config leak"),
    ("{{self._TemplateReference__context}}",  r"_TemplateReference__context", "Jinja2 ctx leak"),
]
REDIRECT_PARAMS = {
...
def _test_ssti(url: str, cookie: str, timeout: int) -> list[dict]:
    ...
    def _fetch(target_url: str) -> str:
        try:
            r = subprocess.run(base_cmd + [target_url], capture_output=True, text=True, timeout=timeout+2)
            return r.stdout or ""
        except Exception:
            return ""

    baseline_body = _fetch(url)

    findings = []
    for param_key in params:
        # Reflection sentinel — if the parameter is never echoed, no template
        # ever sees it and SSTI cannot exist. This single check eliminates
        # most false positives from "49" or "60481729" appearing organically.
...
# Unique canary host — substring-match on plain "evil.com" was tripping on any
# server response that echoed the rejected payload back inside the Location
# header (e.g. `Location: /error?blocked=https://evil.com`). A unique canary
# combined with proper URL-host parsing eliminates that whole class of FPs.
_REDIRECT_CANARY = "venomcanary.evil-redirect.com"
...
def _test_redirect(url: str, timeout: int) -> list[dict]:
    ...
    findings = []
    payloads = [
        f"https://{_REDIRECT_CANARY}/",
        f"//{_REDIRECT_CANARY}/",
        f"///{_REDIRECT_CANARY}/",
        f"/\\/{_REDIRECT_CANARY}/",
        f"https:{_REDIRECT_CANARY}/",
    ]
    canary = _REDIRECT_CANARY.lower()
    ...
    for key in params:
        ...
        for payload in payloads:
            ...
            test_url = urllib.parse.urlunparse(parsed._replace(
                query=urllib.parse.urlencode(test_params, doseq=True)))
            try:
                # GET with header dump (HEAD missed endpoints that only redirect
                # on GET); --max-redirs 0 so we see the first hop only.
                r = subprocess.run(
                    ["curl","-sk","-X","GET","-D","-","-o","/dev/null",
                    ...
                    capture_output=True, text=True, timeout=timeout+2)
                raw = r.stdout or ""
                # Find status code (last response in the dump)
                status = 0
                loc = ""
                ...
                if dest_host == canary or dest_host.endswith("." + canary):
                    findings.append({"type":"open_redirect","severity":"medium","url":url,
                                     "param":key,"payload":payload,"location":loc,"status":status})
